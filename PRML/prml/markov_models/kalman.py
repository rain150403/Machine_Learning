# 是不是用到了还没有实现的那个类state space model？
# 限制噪声服从高斯分布
import numpy as np
from .state_space_model import StateSpaceModel

class Kalman(StataSpaceModel):
	"""
	A class to perform kalman filtering or smoothing
	"""
	def __init__(self, transition, observation, process_noise, measurement_noise, init_state_mean, init_state_cov):
		"""
		...
		assert (
			transition.ndim == observation.ndim 
			== process_noise.ndim == measurement_noise.ndim 
			== init_state_cov.ndim == 2)
		assert (
	...
	def filtering(self, seq):
		...
		kalman filter
		1. prediction 
			p(z_n+1|x_1:n) = \int p(z_n+1|z_n)p(z_n|x_1:n)
		2. filtering
		...
		"""
		kalman_gain = self.init_state_cov @ self.observation.T @ np.linalg.inv(self.observation @ self.init_state_cov @ self.observation.T + self.measurement_noise)
		...
		for s in seq[1:]:
			...
			cov_predict = (
				self.transition @ cov[-1] @ self.transition.T + self.process_noise)
			if np.logical_and.reduce(np.isnan(s)):
				mean.append(mean_predict)
				cov.append(cov_predict)
			else:
				kalman_gain = cov_predict @ self.observation.T @ np.linalg.inv(self.observation @ cov_predict @ self.observation.T + self.measurement_noise)
				mean.append(mean_predict + kalman_gain @ (s - self.observation @ mean_predict))
				cov.append((np.eye(self.ndim_observe) - kalman_gain @ self.observation) @ cov_predict)

...
class VariationalGaussianMixture(RandomVariable): # http://blog.csdn.net/jinping_shi/article/details/59613054

    def __init__(self, n_components=1, alpha0=None, m0=None, W0=1., dof0=None, beta0=1.):
        """
        construct variational gaussian mixture model 构建变分高斯混合模型
        Parameters
        ----------
        n_components : int
            maximum numnber of gaussian components 高斯成分的最大数量
        alpha0 : float
            parameter of prior dirichlet distribution 先验dirichlet分布的参数alpha0
        m0 : float
            mean parameter of prior gaussian distribution 先验高斯分布的参数mean
        W0 : float
            mean of the prior Wishart distribution 先验wishart 分布的mean
        dof0 : float
            number of degrees of freedom of the prior Wishart distribution  先验wishart 分布的自由度的数量
        beta0 : float
            prior on the precision distribution 精度分布的先验
        """
        super().__init__()
    ...
    def classify(self, X):
        """
        index of highest posterior of the latent variable 潜在变量的最高后验的索引
        Parameters
        ----------
    ...
    def classify_proba(self, X):
        ...
        -------
        output : (sample_size, n_components) ndarray
            posterior of the latent variable 潜在变量的后验
        """
    ...
    def student_t(self, X): # 
        ...
        return (
            ...
            * (1 + maha_sq / nu) ** (-0.5 * (nu + self.ndim))
            / (gamma(0.5 * nu) * (nu * np.pi) ** (0.5 * self.ndim)))  # 这里完全按照手写笔记上的公式来的， 这样反观上面的EM算法就能知道每个变量的意思，就能知道整体的意思了

    def _pdf(self, X):
        return (self.alpha * self.student_t(X)).sum(axis=-1) / self.alpha.sum() # 混合高斯模型的概率密度函数怎么是学生T分布的组合？

...
定义超平面（w， b）关于训练数据集T的函数间隔为超平面（w，b）关于T中所有样本点（xi，yi）的函数间隔之最小值， 即：γ = min γi

几何间隔：（当成倍改变w， 和b时， 超平面没变，函数间隔却改变了，所以用||w||来归一化）
Γi = yi (w · xi + b) / ||w||
...
H1, H2 之间的距离称为间隔（margin）， 间隔依赖于分离超平面的法向量w 等于2/||w||。 H1，和H2称为间隔边界（决策边界）。
"""
